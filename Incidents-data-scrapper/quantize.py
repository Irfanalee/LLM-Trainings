...
LLAMA_CPP_DIR = "../code-review-critic/llama.cpp"
...
def main():
    ...
    # Check if llama.cpp exists (try sibling project first, then local)
    ...
    q4_path = f"{OUTPUT_DIR}/incident-responder-{QUANT_METHOD.lower()}.gguf"
    print("\n" + "=" * 60)
...
SYSTEM "You are an expert DevOps engineer and SRE. Analyze the provided error logs, stack traces, or incident descriptions. Provide root cause analysis, severity assessment, step-by-step fixes, and prevention guidance."
...
ollama create incident-responder -f Modelfile

3. Test it:

ollama run incident-responder "Analyze this kubernetes incident:

kubectl describe pod api-server-7d4b8c6f5-x2k9m
State:          Terminated
Reason:         OOMKilled
Exit Code:      137
Restart Count:  5
